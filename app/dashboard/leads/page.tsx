  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white">ðŸŽ¯ Lead Generation Plan</h1>
          <p className="text-gray-400 text-sm mt-1">ICP filters, inbound channels, cold outreach, and qualification rules</p>
        </div>
        <div className="flex gap-3">
          {plan && <button onClick={() => router.push('/dashboard/export')} className="px-4 py-2 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-300 text-sm">Next: Export Plan â†’</button>}
          <button onClick={generate} disabled={loading} className="px-5 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-sm font-medium">
            {loading ? 'Generating...' : plan ? 'â†» Regenerate' : 'âš¡ Generate Lead Gen Plan'}
          </button>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-gray-900 border border-orange-800 rounded-xl p-6">
              <h3 className="text-white font-semibold mb-4">ðŸ” ICP Filters</h3>
              <div className="space-y-3 text-sm">
                ...
                <div>
                  ...
                  <ul className="space-y-1">{plan.icpFilters?.signals?.map((s, idx) => <li key={idx} className="text-gray-400 text-xs">â€¢ {s}</li>)}</ul>
                </div>
              </div>
            </div>

            <div className="bg-gray-900 border border-green-800 rounded-xl p-6">
              <h3 className="text-white font-semibold mb-4">ðŸ“¥ Inbound Strategy</h3>
              <div className="space-y-3 text-sm">
                <div>
                ...
                <div>
                  ...
                  <ul className="space-y-1">{plan.inboundStrategy?.leadMagnets?.map((m, i) => <li key={i} className="text-gray-400 text-xs">â€¢ {m}</li>)}</ul>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-900 border border-blue-800 rounded-xl p-6">
            <h3 className="text-white font-semibold mb-4">ðŸ“¤ Outbound Playbook</h3>
            <div className="grid grid-cols-2 gap-6">
              <div>
          ...
          <div className="bg-gray-900 border border-indigo-800 rounded-xl p-6">
            <h3 className="text-white font-semibold mb-4">âœ… Qualification Rules</h3>
            <div className="space-y-2">

...
export async function POST(req: NextRequest) {
  try {
    ...
    // Inject Razorpay credentials from workspace settings
    if (settings.razorpayKeyId) {
      process.env.RAZORPAY_KEY_ID = settings.razorpayKeyId as string
    }
    if (settings.razorpayKeySecret) {
      process.env.RAZORPAY_KEY_SECRET = settings.razorpayKeySecret as string
    }
